...
class MatchPlugin implements ManialinkPageAnswerListener, CallbackListener, CommandListener, TimerListener, CommunicationListener, Plugin
{
    ...
    public function load(ManiaControl $maniaControl)
    {
        ...
        $this->maniaControl->getSettingManager()->initSetting(
            ...
            array("Cup", "Rounds", "TimeAttack", "Laps", "Team")
        );
        $this->maniaControl->getSettingManager()->initSetting(
            $this,
        ...
        $this->maniaControl->getSettingManager()->initSetting(
            $this,
            self::SETTING_MATCH_MATCH_CUSTOMPOINTSREPARTITION,
            true
        );
...
//        );
        ...
        $this->maniaControl->getCommandManager()->registerCommandListener(
            ...
            'onCommandSetPause',
            true,
            'Set pause during a match. [time] in seconds can be added to force another value'
        );
        ...
        $this->maniaControl->getCommandManager()->registerCommandListener(
            ...
            'onCommandUnsetPause',
            true,
            'End the pause during a match.'
        );

        $this->maniaControl->getCommandManager()->registerCommandListener(
            'pause',
            $this,
            'onCommandSetPausePlayer',
            false,
            'Set pause during a match.'
        );
...
//                    Logger::log("2");
                    ...
                    if ($this->maniaControl->getSettingManager()->getSettingValue(
                        ...
                        if ($this->maniaControl->getPlayerManager()->getPlayerCount() < ($this->maniaControl->getSettingManager()->getSettingValue(
                                    $this,
                                    self::SETTING_MATCH_MATCH_NBWINNERS
                                ) + 1)) {
                            $nbwinners = ($this->maniaControl->getPlayerManager()->getPlayerCount() - 1);
                        } else {
                            $nbwinners = $this->maniaControl->getSettingManager()->getSettingValue(
                                $this,
                                self::SETTING_MATCH_MATCH_NBWINNERS
                            );
                        }

                        $pointlimit = $this->maniaControl->getSettingManager()->getSettingValue(
                            $this,
...
//                    Logger::log("Cup mode");
                        $loadedSettings = array(
                            self::SETTING_MATCH_MATCH_POINTS => (int)$pointlimit,
...
//                    Logger::log("Rounds mode");
                        $loadedSettings = array(
                            self::SETTING_MATCH_MATCH_CUSTOMPOINTSREPARTITION => (boolean)$this->maniaControl->getSettingManager(
                            )->getSettingValue($this, self::SETTING_MATCH_MATCH_CUSTOMPOINTSREPARTITION),
                            ...
                            self::SETTING_MATCH_MATCH_USETIEBREAK => (boolean)$this->maniaControl->getSettingManager(
                            )->getSettingValue($this, self::SETTING_MATCH_MATCH_USETIEBREAK),
                            self::SETTING_MATCH_MATCH_CUMULATEPOINTS => (boolean)$this->maniaControl->getSettingManager(
                            )->getSettingValue($this, self::SETTING_MATCH_MATCH_CUMULATEPOINTS),
                            ...
                            self::SETTING_MATCH_MATCH_POINTSGAP => (int)$this->maniaControl->getSettingManager(
                            )->getSettingValue($this, self::SETTING_MATCH_MATCH_POINTSGAP),
                        );
...
//                    Logger::log("4");
                    ...
                    $respawn = (!isset($this->scriptSettings['S_AllowRespawn']) || is_null(
                            $this->scriptSettings['S_AllowRespawn']
                        )) ? '0' : $this->scriptSettings['S_AllowRespawn'];
                    ...
                    if ($script['CurrentValue'] == "Cup.Script.txt") {
                        ...
                        $matchdetail = "S_PointsLimit => " . $this->scriptSettings['S_PointsLimit'] . ", S_AllowRespawn => " . $respawn .
                        ...
                        $matchdetail = "S_PointsLimit => ".$this->scriptSettings['S_PointsLimit'].", S_CumulatePoints => ".$this->scriptSettings['S_CumulatePoints'].", S_AllowRespawn => ".$respawn.
                            ",S_RoundsPerMap => ".$this->scriptSettings['S_RoundsPerMap'].", S_MapsPerMatch => ".$this->scriptSettings['S_MapsPerMatch'].
                            ",S_WarmUpNb => ".$this->scriptSettings['S_WarmUpNb'].", S_FinishTimeout => ".$this->scriptSettings['S_FinishTimeout'].", S_UseCustomPointsRepartition => ".$this->scriptSettings['S_UseCustomPointsRepartition'];
...
//        }
    ...
    {
        $options = array(
            "Nb Winners" => (!isset($this->scriptSettings['S_NbOfWinners']) || is_null(
                    $this->scriptSettings['S_NbOfWinners']
                )) ? '0' : $this->scriptSettings['S_NbOfWinners'],
            "Points limit" => (!isset($this->scriptSettings['S_PointsLimit']) || is_null(
                    $this->scriptSettings['S_PointsLimit']
                )) ? '0' : $this->scriptSettings['S_PointsLimit'],
            "Points Repartition" => $this->maniaControl->getSettingManager()->getSettingValue(
                $this,
                self::SETTING_MATCH_MATCH_POINTSREPARTITION
            ),
            "Allow Respawn" => (!isset($this->scriptSettings['S_AllowRespawn']) || is_null(
                    $this->scriptSettings['S_AllowRespawn']
                )) ? '0' : $this->scriptSettings['S_AllowRespawn'],
            "Gamemode" => $this->maniaControl->getClient()->getModeScriptInfo()->name,
            "Rounds per map" => (!isset($this->scriptSettings['S_RoundsPerMap']) || is_null(
                    $this->scriptSettings['S_RoundsPerMap']
                )) ? '0' : $this->scriptSettings['S_RoundsPerMap'],
...
//        });
    ...
    public function SetPoints(Player $player, $points)
    {
        ...
        if ($player) {
            ...
            $this->maniaControl->getChat()->sendChat(
                '$<$0f0$o Player $z' . $player->nickname . ' $z$o$0f0has now ' . $points . ' points!$>'
            );
            $this->livePlugin = $this->maniaControl->getPluginManager()->getPlugin(self::DEFAULT_TRACKMANIALIVE_PLUGIN);
            if ($this->livePlugin) {
    ...
    public function onCommandSetPause(array $chatCallback)
    {
        if (($this->matchStarted) && ($this->getMatchMode() != "TimeAttack")) {
            ...
            if (isset($text[1]) && $text[1] != "") {
                $this->maniaControl->getChat()->sendChat(
                    '$<$0f0$o Admin force a break for ' . $text[1] . ' seconds!$>'
                );
                if ($this->maniaControl->getSettingManager()->getSettingValue(
                        $this,
                ...
                $this->maniaControl->getChat()->sendChat(
                    '$<$0f0$o Admin force a break for ' . $this->maniaControl->getSettingManager()->getSettingValue(
                        $this,
                        self::SETTING_MATCH_PAUSE_DURATION
                    ) . ' seconds!$>'
                );
                if ($this->maniaControl->getSettingManager()->getSettingValue(
                        $this,
    ...
    private function setPause($admin = false, $time = null)
    {
        ...
        if ($time === null) {
            $this->pausetimer = $this->maniaControl->getSettingManager()->getSettingValue(
                $this,
                self::SETTING_MATCH_PAUSE_DURATION
            );
        } else {
...
//        var_dump($pausestatus);
    ...
    public function onCommandSetPausePlayer(array $chatCallback, Player $player)
    {
        if (($this->matchStarted) && ($this->getMatchMode() != "TimeAttack") && (!$player->isSpectator && !$player->isPureSpectator)) {
            if ($this->playerpause[$player->login] < $this->maniaControl->getSettingManager()->getSettingValue(
                    $this,
                    self::SETTING_MATCH_PAUSE_MAXNUMBER
                )) {

                if ($this->pauseasked === false && $this->pauseon === false) {

                    $this->maniaControl->getChat()->sendChat(
                        '$<$0f0$o Player $z' . $player->nickname . ' $z$o$0f0ask for a break of ' . $this->maniaControl->getSettingManager()->getSettingValue(
                            $this,
                            self::SETTING_MATCH_PAUSE_DURATION
                        ) . ' seconds! It will start after this round.$>'
                    );
...
//                $this->setPause();
                    ...
                    $this->maniaControl->getChat()->sendChat(
                        '$<$f00$o Pause already asked by a player!$>',
                        $player->login
                    );
                }
            } else {
                $this->maniaControl->getChat()->sendChat(
                    '$<$f00$o Player $z' . $player->nickname . ' $z$o$f00already ask too many breaks!$>',
                    $player->login
                );
...
//                                Logger::log("Winner");
                                ...
                                } elseif (($points + $roundpoints) >= $this->maniaControl->getSettingManager()->getSettingValue(
                                        $this,
                                        self::SETTING_MATCH_MATCH_POINTS
...
//                                );
                ...
                if (($realRound && $this->maniaControl->getSettingManager()->getSettingValue(
                        ...
                        $this,
                        self::SETTING_MATCH_MATCH_MODE
                    ) == "Rounds" || $this->maniaControl->getSettingManager()->getSettingValue(
                        $this,
                        self::SETTING_MATCH_MATCH_MODE
                    ) == "Team"
                ) {
                ...
                if ($this->type == "TA" AND !$this->alreadyDoneTA) {
                    ...
                    $mysqli->query($query);
                    if ($this->livePlugin) {
                        $this->livePlugin->updateTATimes(
                            $this->scores,
                            $this->maniaControl->getMapManager()->getCurrentMap(),
                            $this->nbmaps
                        );
                    }
    ...
    {
        return $this->maniaControl->getSettingManager()->getSettingValue(
            $this,
            self::SETTING_MATCH_MATCH_POINTSREPARTITION
        );
    }
        ...
        if ($this->matchStarted) {
            if ($this->maniaControl->getSettingManager()->getSettingValue(
                ...
                if ($this->settings_nbwinners == $this->nbwinners
                    ...
                    $this->MatchEnd();
                } elseif (count($this->playerswinner) == $this->settings_nbwinners AND count(
                        $this->playerswinner
                    ) > 0) {
                    $this->MatchEnd();
                }
            ...
            if (($this->pauseasked) && $this->maniaControl->getSettingManager()->getSettingValue(
                    $this,
                ...
                $this->pauseaskedbyplayer = "";
                $this->maniaControl->getChat()->sendChat(
                    '$<$f00$o Pause cancelled because of a WarmUp!$>'
                );
            }
        }
    }
